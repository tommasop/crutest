...
test "exit code of successful run" do
  %x{./bin/crutest test/fixtures/success.rb}
  assert_equal 0, $?.to_i
end
...
test "output of failed run" do
  expected = "\n" +
             ...
             "Crutest::AssertionFailed: expression returned false\n\n"
...
test "exit code of failed run" do
  %x{./bin/crutest test/fixtures/failure.rb}
...
test "output of an assertion with custom message" do
  expected = "\n" +
             ...
             "Crutest::AssertionFailed: not empty\n\n"
...
test "runs by given scope and test names" do
  %x{./bin/crutest test/fixtures/only_run_given_scope_name.rb -s scope -o test}
